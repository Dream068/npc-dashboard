...
function ForgotPassword() {
  ...
  const moveNextStep = () => {
    history.push(routes.FORGOT_PASSWORD_NEXTSTEP);
  };
  return (
    <div className={classes.contentRoot}>
      ...
      <div className={classes.passwordButton}>
        <Button fontSize="26px" width="400px" height="70px" radius="50px" onClick={moveNextStep}>
          {t('forgetPassword')}
        </Button>
      </div>
      <div className={classes.youWill}>
        <Typography variant="h5" color="secondary">{t('you_will_get')}</Typography>
      </div>
    </div>
  );
}
